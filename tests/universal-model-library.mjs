...
const calls=[],oldFetch=globalThis.fetch;try{globalThis.fetch=async(url,init={})=>{const body=init.body?JSON.parse(init.body):{};calls.push({url:String(url),body});if(String(url).includes("IntrospectToken"))return new Response(JSON.stringify({active:true,username:"tester",userId:1,scope:"read write"}),{status:200,headers:{"content-type":"application/json"}});if(String(url).includes("SaveKernel"))return new Response(JSON.stringify({ref:"tester/model-recipe",versionNumber:1}),{status:200,headers:{"content-type":"application/json"}});throw new Error("UNEXPECTED_FETCH:"+url)};const out=await dispatch({KAGGLE_API_TOKEN:"secret"},{task_id:"model-recipe-test",profile:"model",input:{model_recipe:{model_id:"policy_economics.econometrics.ols",args:{X:[[1],[2],[3]],y:[2,4,6]}}},timeout_seconds:300,gpu:false});assert.equal(out.machine_shape,"cpu");const save=calls.find(x=>x.url.includes("SaveKernel"));assert.ok(save);assert.equal(save.body.enableInternet,false);assert.equal(save.body.isPrivate,true);assert.equal(save.body.enableGpu,false);assert.ok(String(save.body.text).includes("model-recipe"));assert.equal(String(save.body.text).includes("pip install"),false)}finally{globalThis.fetch=oldFetch}
