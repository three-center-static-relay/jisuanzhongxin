...
assert.equal(loc.observed_lbs,false);assert.equal(loc.calibrated,false);assert.equal(loc.calibration_state,"deferred-until-benchmark-library");assert.equal(loc.activity_cells.length,2);assert.ok(loc.trade_area);assert.ok(Math.abs(Object.values(loc.trade_area.destination_expected_flow).reduce((s,x)=>s+x,0)-3000)<1e-6);

const project=runLocalModel("npv",{discount_rate:0.1,cash_flows:[-100,60,60]});assert.ok(project.npv>0);
const irr=runLocalModel("irr",{cash_flows:[-100,60,60]});assert.ok(irr.irr>0);
const eoq=runLocalModel("eoq",{annual_demand:10000,order_cost:50,holding_cost_per_unit_year:2});assert.ok(eoq.economic_order_quantity>0);
...
const credit=runLocalModel("credit_expected_loss",{exposures:[{id:"x",pd:0.02,lgd:0.4,ead:100000}]});assert.equal(credit.total_expected_loss,800);
...
console.log(JSON.stringify({ok:true,suite:"universal-model-library",industry_packs:summary.industry_pack_count,local_models:Object.keys(LOCAL_MODELS).length,recipe_methods:recipes.methods.length,arbitrary_code:false,calibration_deferred:true,observed_lbs_claim:false}));
