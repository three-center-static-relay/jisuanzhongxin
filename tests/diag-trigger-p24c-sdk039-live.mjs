...
// Status-only submit-stage classifier. Never calls the P24c acceptance trigger.
...
try {
  ...
  assert.ok(["aistudio_submit_returned","baidu_submitted"].includes(body.task?.bridge_stage));
  ...
  console.log(JSON.stringify({ok:true,suite:"diag-p24c-sdk039-submit-returned-group",failure_class:body.task.failure_class,bridge_stage:body.task.bridge_stage}));
