...
console.log(JSON.stringify({http:r.status,expected,configured:body.configured===true,task_present:Boolean(t),status:t?.status||null,pipeline_id_present:Boolean(t?.circleci_pipeline_id),baidu_job_id_present:t?.baidu_job_id_present===true,bridge_stage:t?.bridge_stage||null,failure_class:t?.failure_class||null,verification_ok:t?.verification?.ok===true,result_digest_present:Boolean(t?.result_digest),bridge_result_retrieved:t?.bridge_result_retrieved===true,output_diag:t?.output_diag||null,error:t?.error||null}));
...
else if(expected==="diag")assert.ok(t?.output_diag,"output_diag missing");
