...
assert.equal((wrangler.match(/\*\/5 \* \* \* \*/g)||[]).length,1,"Acceptance build may contain exactly one temporary 5-minute Cron");
assert.ok(entry.includes('controller.cron===STUDIO_ACCEPTANCE_CRON'),"Temporary acceptance Cron must be isolated from normal scheduled maintenance");
...
console.log(JSON.stringify({ok:true,suite:"modelscope-studio-lite-cpu-runner-contract",module_imported:true,target_hardware:"platform/2v-cpu-16g-mem",nominal_cpu:2,nominal_memory_gb:16,min_effective_cpu:1.9,min_effective_memory_gib:14,free_only:true,paid_fallback:false,private_studio:true,phased_runner:true,idempotent_upload:true,stop_independent_of_hardware_catalog:true,public_write_surface:false,internal_workflow_control:true,temporary_acceptance_cron:true}));
