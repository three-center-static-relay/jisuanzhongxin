...
for(const f of ["built_land_cover","green_space","air_quality_exposure"])assert.equal(meta.features.includes(f),true);
...
const args={
  source_receipts:[{source:"worldpop",digest_sha256:digest},{source:"esa_worldcover",digest_sha256:"b".repeat(64)},{source:"openaq",digest_sha256:"c".repeat(64)},{source:"baidu_traffic",digest_sha256:"d".repeat(64)}],
  units:[
    {id:"h3:8941b530807ffff",features:{population_market:88,working_age:72,poi_supply:75,poi_diversity:82,building_intensity:79,nonresidential_built:84,built_land_cover:86,green_space:35,air_quality_exposure:32,night_activity:76,walk_accessibility:90,transit_accessibility:86,competition_pressure:70,hazard_risk:15}},
    {id:"h3:8941b53080fffff",features:{population_market:73,working_age:68,poi_supply:55,poi_diversity:64,building_intensity:61,built_land_cover:67,green_space:60,air_quality_exposure:18,night_activity:58,walk_accessibility:70,transit_accessibility:62,competition_pressure:38,hazard_risk:12}}
  ]
};
for(const model of ["geospatial_commercial.spatial_feature_fusion","geospatial_commercial.site_ranking","geospatial_commercial.white_space"]){
  ...
  assert.equal(script.includes("air_quality_exposure"),true);
  assert.equal(script.includes("built_land_cover"),true);
...
console.log(JSON.stringify({ok:true,suite:"geospatial-commercial-domain",methods:meta.methods,reused_shared_models:meta.reused_shared_models,environment_features:["built_land_cover","green_space","air_quality_exposure"],network:false,observed_lbs:false}));
