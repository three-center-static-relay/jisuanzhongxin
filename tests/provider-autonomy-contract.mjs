...
assert.match(autonomy,/free_only:true/);
assert.match(autonomy,/paid_fallback:false/);
assert.match(autonomy,/scheduled_gpu_canary:false/);
assert.match(autonomy,/quarantine_after_consecutive_failures:2/);
assert.match(autonomy,/recover_after_consecutive_successes:2/);
assert.match(autonomy,/user_routine_maintenance_required:false/);
assert.match(autonomy,/candidate_runtime:"paddle2\.5_py3\.10"/);
assert.match(autonomy,/quarantined_runtimes:\["paddle2\.6_py3\.10"\]/);
assert.match(autonomy,/production_runtime:null/);
...
assert.match(wrangler,/"triggers"\s*:\s*\{"crons"\s*:\s*\["17 4 \* \* \*"\]\}/);
