...
assert.match(wrangler,/"crons"\s*:\s*\["17 4 \* \* \*",\s*"\* \* \* \* \*"\]/);
...
for(const pattern of [/baidu_payment:"coupon"/,/free_only:true/,/paid_fallback:false/,/acoin_allowed:false/,/sdk_pinned:"aistudio-sdk==0\.3\.8"/,/runtime_candidate:"paddle2\.4_py3\.7"/,/runtime_candidate_state:"DEGRADED"/,/automatic_candidate_execution:false/,/automatic_same_failure_retry:false/,/route_eligible:configured&&e2eVerified/])assert.match(baidu,pattern);
...
console.log(JSON.stringify({ok:true,suite:"provider-autonomy-contract",free_only:true,daily_control_plane:true,scheduled_gpu_canary:false,temporary_p24b_one_shot:true,route_requires_live_health:true,baidu_candidate:"paddle2.4_py3.7",baidu_candidate_state:"DEGRADED",manual_single_retest_only:true,automatic_candidate_execution:false,automatic_same_failure_retry:false,production_runtime:null,bootstrap_sentinel:true}));
