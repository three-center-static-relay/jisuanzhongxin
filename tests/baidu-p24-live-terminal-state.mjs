...
try{
  ...
  assert.ok(["diagnostic_dispatching","diagnostic_submitted","running"].includes(String(b.status||"")));
  assert.equal(b.secrets_redacted,true);
  assert.equal(b.source_job_id_exposed,false);
  assert.equal(b.result_body_exposed,false);
  console.log(JSON.stringify({ok:true,suite:"baidu-p24-live-nonterminal-state",nonterminal:true,circleci_pipeline_id_present:b.circleci_pipeline_id_present===true,baidu_job_id_present:b.baidu_job_id_present===true,network:true}));
