...
// retrigger: 2026-08-15T20:35+08:00; logic unchanged
...
const sr=await fetch(URL,{method:"POST",headers:{accept:"application/json"}});
const s=await read(sr);
console.log(JSON.stringify({phase:"dispatch",http:sr.status,ok:s.ok===true,status:s.status||s.task?.status||null,already_completed:s.already_completed===true,pipeline_id_present:Boolean(s.circleci_pipeline_id),error:s.error||null}));
assert.ok([200,202].includes(sr.status),`dispatch HTTP ${sr.status}: ${JSON.stringify(s)}`);
assert.equal(s.ok,true,`dispatch rejected: ${JSON.stringify(s)}`);
...
while(Date.now()<deadline){
  ...
  if(t.status==="completed"){
    ...
    console.log(JSON.stringify({ok:true,suite:"baidu-fetch-after-path-fix",status:"completed",verification_ok:true,result_retrieved:true}));
    process.exit(0);
  }
