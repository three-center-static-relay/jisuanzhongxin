...
const network=setupServer(
  http.post("https://bridge.test/dispatch",async({request})=>{dispatchCalls++;const b=await request.json(),scenario=b?.input?.scenario||"terminal";if(scenario==="running")return HttpResponse.json({status:"running",status_url:`https://bridge.test/status/${b.task_id}`,cancel_url:`https://bridge.test/cancel/${b.task_id}`},{status:202});if(scenario==="terminal-failed")return HttpResponse.json({status:"failed",terminal:true,error:"synthetic failure"});if(scenario==="terminal-cancelled")return HttpResponse.json({status:"cancelled",terminal:true});return HttpResponse.json({status:"completed",terminal:true,result:{profile:b.profile,gpu:b.gpu,timeout_seconds:b.timeout_seconds}})}),
...
async function reset(){await within(server.reset(),5000,"reset");dispatchCalls=0}
let exitCode=0;
try{
  await within(server.listen(),10000,"listen");

  await reset();const holder=await run("cx-holder","running","core",300);assert.equal(holder.status,202);const unique=await within(Promise.all(Array.from({length:256},(_,i)=>run(`cx-u-${i}`))),25000,"256-unique");assert.equal(unique.filter(x=>x.status===409&&x.body?.error==="BUSY").length,199);assert.equal(unique.filter(x=>x.status===429&&x.body?.error==="RATE_LIMITED").length,57);assert.equal(dispatchCalls,1);const holderStatus=await post("/v1/status",{task_id:"cx-holder"});assert.equal(holderStatus.status,200);assert.equal(holderStatus.body?.lock_released,true);

  await reset();const dupHolder=await run("cx-dup","running","core",300);assert.equal(dupHolder.status,202);const dup=await within(Promise.all(Array.from({length:512},()=>run("cx-dup"))),30000,"512-duplicate");assert.equal(dup.filter(x=>x.status===409&&x.body?.error==="DUPLICATE_TASK").length,199);assert.equal(dup.filter(x=>x.status===429&&x.body?.error==="RATE_LIMITED").length,313);assert.equal(dispatchCalls,1);const dupStatus=await post("/v1/status",{task_id:"cx-dup"});assert.equal(dupStatus.status,200);assert.equal(dupStatus.body?.lock_released,true);
  ...
  console.log(JSON.stringify({ok:true,suite:"compute-extreme2",unique_contenders:256,unique_busy:199,unique_rate_limited:57,duplicate_contenders:512,duplicate_rejected:199,duplicate_rate_limited:313,lease_boundary_seconds:31,profiles:8,rate_total:2000,health_total:1024,tests:["256-running-lock-overload","512-running-duplicate-overload","31s-running-lease-no-overlap","terminal-failure-not-completed","terminal-cancel-not-completed","all-profiles","status-release","cancel-release","2000-overload","1024-health-burst"]}));
