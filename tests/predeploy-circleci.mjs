...
import {baiduCircleCIMeta,normalizeBaiduInput,triggerBaiduBridge} from "../src/baidu-circleci.js";
...
const ready=baiduCircleCIMeta({CIRCLECI_API_TOKEN:"x",CIRCLECI_PROJECT_SLUG:"circleci/org/project",CIRCLECI_PIPELINE_DEFINITION_ID:"def",BAIDU_BRIDGE_SHARED_SECRET:"secret-value",BAIDU_CIRCLECI_E2E_VERIFIED:"true"});
...
try{
  ...
  const out=await triggerBaiduBridge({CIRCLECI_API_TOKEN:"circle-token",CIRCLECI_PROJECT_SLUG:"circleci/org/project",CIRCLECI_PIPELINE_DEFINITION_ID:"definition-1",CIRCLECI_CONFIG_BRANCH:"main",BAIDU_BRIDGE_SHARED_SECRET:"secret-value"},{op:"SUBMIT",task_id:"task-001"});
  ...
  assert.equal(JSON.stringify(seen).includes("secret-value"),false);
}finally{globalThis.fetch=oldFetch}

console.log(JSON.stringify({ok:true,suite:"predeploy-circleci",fail_closed:true,coupon_only:true,fixed_v100:true,arbitrary_code:false,network:false}));
