...
try{
  ...
  assert.equal(b.task.circleci_workflow_status,"failed");
  assert.equal(b.task.sdk_selftest_passed,false);
  assert.equal(b.task.gpu,false);
  assert.equal(b.task.compute_credit_used,false);
  assert.equal(b.task.production_promoted,false);
  console.log(JSON.stringify({ok:true,suite:"baidu-sdk039-failure-layer",task_status:"failed",circleci_workflow_status:"failed",sdk_selftest_passed:false,gpu:false,compute_credit_used:false}));
