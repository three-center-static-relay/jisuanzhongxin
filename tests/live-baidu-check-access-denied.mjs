...
const EXPECTED="BAIDU_JOB_ACCESS_DENIED";
...
const deadline=Date.now()+150000;let last=null;while(Date.now()<deadline){await sleep(10000);const r=await fetch(CHECK,{headers:{accept:"application/json"}});const b=await read(r);last=b;const t=b?.task||{};console.log(JSON.stringify({status:t.status||null,failure_class:t.failure_class||null}));assert.equal(r.status,200);if(t.status==="completed"){assert.equal(t.verification_ok,true);assert.equal(t.result_digest_present,true);assert.equal(t.bridge_result_retrieved,true);process.exit(0)}if(t.status==="failed"){assert.equal(t.failure_class,EXPECTED,`diagnostic class=${t.failure_class}`);process.exit(0)}}throw new Error(`CHECK_DIAGNOSTIC_TIMEOUT:${JSON.stringify(last)}`);
