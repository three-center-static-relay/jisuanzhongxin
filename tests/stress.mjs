...
function completedLog(slug){const task_id=taskIdFromSlug(slug);return `THREE_CENTER_RESULT:${JSON.stringify({ok:true,task_id,profile:"core",accelerator:"cpu",pi:3.14,linear_residual:0})}`}
...
const network=setupServer(
  ...
  http.post("https://api.kaggle.com/v1/kernels.KernelsApiService/SaveKernel",async({request})=>{
    ...
    if(title.includes("compute-holder")||title.includes("compute-duplicate")){holdEnteredResolve();await holdRelease;}
    if(title.includes("upstream-fail"))return HttpResponse.json({message:"synthetic-upstream-failure"},{status:503});
    if(title.includes("oversize-dispatch"))return HttpResponse.json({padding:"x".repeat(2_100_000)});
  ...
  http.post("https://api.kaggle.com/v1/kernels.KernelsApiService/GetKernelSessionStatus",async({request})=>{
    ...
    return HttpResponse.json({status:slug.includes("cancel-running")?"CANCELLED":"COMPLETE"});
  }),
  http.post("https://api.kaggle.com/v1/kernels.KernelsApiService/ListKernelSessionOutput",async({request})=>{
    const b=await request.json(),slug=String(b?.kernelSlug||"");
    return HttpResponse.json({log:completedLog(slug),files:[]});
  }),
...
try{
  ...
  await reset();armHold();
  const dupFirst=run("compute-duplicate");await within(holdEntered,5000,"dup-enter");
  const dup=await within(Promise.all(Array.from({length:64},()=>run("compute-duplicate"))),12000,"duplicate-contenders");
  assert.equal(dup.filter(x=>x.status===409&&x.body?.error==="DUPLICATE_TASK").length,64);
  letGo();assert.equal((await within(dupFirst,5000,"dup-finish")).status,202);

  await reset();
  assert.equal((await run("running-id")).status,202);
  assert.equal((await run("blocked-while-running")).status,409);
  const st=await post("/v1/status",{task_id:"running-id"});
  assert.equal(st.status,200);assert.equal(st.body?.ok,true);assert.equal(st.body?.status,"completed");
  assert.equal((await run("after-status-release")).status,202);

  await reset();
  const unavailable=await run("upstream-fail");
  assert.equal(unavailable.status,503);assert.equal(unavailable.body?.error,"synthetic-upstream-failure");
  assert.equal((await run("after-upstream-fail")).status,202);

  await reset();
  const oversized=await run("oversize-dispatch");
  assert.equal(oversized.status,502);assert.equal(oversized.body?.error,"KAGGLE_RESPONSE_TOO_LARGE");
  assert.equal((await run("after-oversize-dispatch")).status,202);

  await reset();
  assert.equal((await run("cancel-running")).status,202);
  const cancel=await post("/v1/cancel",{task_id:"cancel-running"});
  assert.equal(cancel.status,202);assert.equal(cancel.body?.lock_retained,true);
  assert.equal((await run("blocked-during-cancel")).status,409);
  const cancelled=await post("/v1/status",{task_id:"cancel-running"});
  ...
  await reset();
  const badProfile=await run("bad-profile","arbitrary-shell");
  assert.equal(badProfile.status,400);assert.equal(badProfile.body?.error,"INVALID_REQUEST");
  const badProvider=await post("/v1/run",{task_id:"bad-provider",profile:"core",provider:"arbitrary"});
  assert.equal(badProvider.status,400);assert.equal(badProvider.body?.error,"INVALID_REQUEST");
  const big={task_id:"huge",profile:"core",input:{padding:"x".repeat(70000)}};
  const hr=await server.fetch(`${INTERNAL}/v1/run`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(big)});
  assert.equal(hr.status,413);

  await reset();
  const burst=await within(Promise.all(Array.from({length:220},(_,i)=>run(`rate-${i}`,"invalid-profile"))),18000,"rate-burst");
  ...
  console.log(JSON.stringify({ok:true,suite:"compute-stress-current-kaggle-control-plane",concurrency_contenders:64,duplicate_contenders:64,rate_burst:220,health_burst:256,tests:["internal-route-policy","single-lock","duplicate-id","running-lock","terminal-status-release","upstream-failure-release","official-api-response-size-cap","cancel-lock-retention-and-release","profile-deny","provider-deny","body-limit","rate-limit","read-burst"],bridge_test_removed:true,kaggle_official_api_mocked:true,kaggle_mcp_mocked:true}));
