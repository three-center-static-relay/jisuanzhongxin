...
const network=setupServer(
  ...
  http.post("https://api.kaggle.com/v1/kernels.KernelsApiService/SaveKernel",async({request})=>{const b=await request.json(),t=String(b?.newTitle||"");if(t==="three-center-cpu-upstream-fail")return HttpResponse.json({message:"synthetic-upstream-failure"},{status:503});if(t==="three-center-cpu-oversize-dispatch")return HttpResponse.json({padding:"x".repeat(2_100_000)});return HttpResponse.json({ref:b.slug,versionNumber:1,url:`https://www.kaggle.com/code/${b.slug}`});}),
  http.post("https://api.kaggle.com/v1/kernels.KernelsApiService/GetKernelSessionStatus",async({request})=>{const b=await request.json(),slug=String(b?.kernelSlug||"");return HttpResponse.json({status:slug==="three-center-cpu-cancel-running"?"CANCELLED":"COMPLETE"});}),
...
try{
  ...
  console.log(JSON.stringify({ok:true,suite:"compute-stress-stage-c-fixed",rate_burst:220,tests:["upstream-failure-release","official-api-response-size-cap","cancel-lock-retention","cancel-terminal-release","rate-limit"],fault_injection_exact_match:true}));
