...
const network=setupServer(
  http.post("https://bridge.test/dispatch",async({request})=>{
    const b=await request.json(),scenario=b?.input?.scenario||"terminal";
    if(scenario==="hold"){holdEnteredResolve();await holdRelease;return HttpResponse.json({status:"completed",terminal:true,result:{ok:true}})}
    if(scenario==="running")return HttpResponse.json({status:"running",status_url:`https://bridge.test/status/${b.task_id}`,cancel_url:`https://bridge.test/cancel/${b.task_id}`},{status:202});
    if(scenario==="cross-origin")return HttpResponse.json({status:"running",status_url:"https://evil.test/status",cancel_url:`https://bridge.test/cancel/${b.task_id}`},{status:202});
    if(scenario==="upstream-503")return HttpResponse.json({error:"synthetic-bridge-failure"},{status:503});
    return HttpResponse.json({status:"completed",terminal:true,result:{profile:b.profile,gpu:b.gpu}});
  }),
...
try{
  ...
  await reset();const badProfile=await run("bad-profile","terminal","arbitrary-shell");assert.equal(badProfile.status,400);assert.equal(badProfile.body?.error,"INVALID_REQUEST");const huge={task_id:"huge",profile:"core",input:{padding:"x".repeat(70000)}};const hr=await server.fetch("/v1/run",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(huge)});assert.equal(hr.status,413);
