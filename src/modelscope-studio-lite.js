...
const REVISION="studio-lite-runtime-v1-20260817";
...
function hardwareRows(raw){
  const direct=raw?.hardware||raw?.data?.hardware||raw?.data?.data?.hardware;
  if(Array.isArray(direct))return direct;
  for(const o of objects(raw)){if(Array.isArray(o?.hardware))return o.hardware}
  return[];
}
function targetHardware(raw){
  for(const h of hardwareRows(raw)){
    const name=str(h?.name),type=str(h?.resource_type||h?.resourceType).toLowerCase();
    if(name===TARGET_HARDWARE&&type==="free")return{name,resource_type:type,cpu:Number(h?.cpu||TARGET_CPU),memory_gb:Number(h?.memory||TARGET_MEMORY_GB),has_stock:h?.has_stock!==false};
  }
  return null;
}
...
function operation(path,content){const size=new TextEncoder().encode(content).length;return{action:"create",path,type:"normal",size,sha256:"",content:b64(content),encoding:"base64"}}
...
async function commitApp(t,owner){return req(`${LEGACY}/repos/studios/${encodeURIComponent(owner)}/${encodeURIComponent(REPO_NAME)}/commit/master`,{method:"POST",headers:legacyHeaders(t),body:{commit_message:`Three-center Studio Lite ${REVISION}`,actions:[operation("app.py",appPy())]},timeout:30000})}
...
async function readiness(env={}){
  ...
  const [me,hw]=await Promise.all([
    req(`${OPENAPI}/users/me`,{headers:headers(t,false)}),
    req(`${OPENAPI}/studios/hardware?sdk_type=${encodeURIComponent(SDK_TYPE)}`,{headers:headers(t,false)})
  ]);
  const owner=me.ok?username(payload(me)):null,target=hw.ok?targetHardware(hw.data):null;
  const ok=me.ok&&hw.ok&&Boolean(owner)&&Boolean(target)&&target.has_stock!==false;
  return{ok,configured:true,authenticated:me.status!==401&&me.status!==403,owner,target,identity_status:me.status,hardware_status:hw.status,error_class:ok?null:!me.ok?`MODELSCOPE_IDENTITY_HTTP_${me.status}`:!owner?"MODELSCOPE_OWNER_UNRESOLVED":!hw.ok?`MODELSCOPE_HARDWARE_HTTP_${hw.status}`:!target?"TARGET_FREE_HARDWARE_UNAVAILABLE":"TARGET_FREE_HARDWARE_OUT_OF_STOCK"};
}
...
export async function runModelScopeStudioLiteBootstrap(env={}){
  ...
  if(!d.ok){
    ...
    const commit=await commitApp(t,r.owner);if(!commit.ok){await stop(t,r.owner);return{ok:false,stage:"upload",create_http_status:c.status,upload_http_status:commit.status,error_class:`MODELSCOPE_STUDIO_COMMIT_HTTP_${commit.status}`,hardware:r.target,free_only:true,paid_fallback:false,secrets_redacted:true}}
  }else{
    const prior=await logs(t,r.owner),pr=prior.ok?parseReceipt(payload(prior)):null;
    if(receiptPass(pr)){await stop(t,r.owner);return{ok:true,stage:"already-verified",hardware:r.target,runtime_receipt:cleanReceipt(pr),free_only:true,paid_fallback:false,secrets_redacted:true}}
  }
  ...
  const dep=await deploy(t,r.owner);if(!dep.ok){await stop(t,r.owner);return{ok:false,stage:"deploy",deploy_http_status:dep.status,error_class:`MODELSCOPE_STUDIO_DEPLOY_HTTP_${dep.status}`,hardware:r.target,free_only:true,paid_fallback:false,secrets_redacted:true}}
