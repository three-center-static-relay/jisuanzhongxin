...
function appPy(){return `import hashlib,json,os,platform,time,math,random\nMARKER=${JSON.stringify(MARKER)}\nREVISION=${JSON.stringify(REVISION)}\nTASK_VAR=${JSON.stringify(TASK_VAR)}\nTASK_REVISION=${JSON.stringify(TASK_REVISION)}\nTASK_MARKER=${JSON.stringify(TASK_MARKER)}\n\ndef _read(p):\n    try:\n        with open(p,'r') as f:return f.read().strip()\n    except Exception:return ''\n\ndef _cpu():\n    logical=os.cpu_count() or 0\n    raw=_read('/sys/fs/cgroup/cpu.max')\n    if raw:\n        p=raw.split()\n        if len(p)>=2 and p[0]!='max':\n            try:return min(float(logical),float(p[0])/float(p[1]))\n            except Exception:pass\n    return float(logical)\n\ndef _mem():\n    host=0.0\n    for line in _read('/proc/meminfo').splitlines():\n        if line.startswith('MemTotal:'):\n            try:host=float(line.split()[1])/1024/1024\n            except Exception:pass\n    lim=_read('/sys/fs/cgroup/memory.max')\n    if lim and lim!='max':\n        try:\n            cg=float(lim)/1024/1024/1024\n            return min(host,cg) if host>0 else cg\n        except Exception:pass\n    return host\n\ndef _task(task):\n    op=str(task.get('op',''))\n    if op=='sum':\n        result=sum(task['values'])\n    elif op=='stats':\n        v=task['values'];n=len(v);s=sum(v);mean=s/n;var=sum((x-mean)**2 for x in v)/n\n        result={'n':n,'sum':s,'mean':mean,'min':min(v),'max':max(v),'variance':var,'std':math.sqrt(var)}\n    elif op=='dot':\n        result=sum(x*y for x,y in zip(task['a'],task['b']))\n    elif op=='matmul':\n        a=task['a'];b=task['b'];bt=list(zip(*b));result=[[sum(x*y for x,y in zip(row,col)) for col in bt] for row in a]\n    elif op=='linear_regression':\n        x=task['x'];y=task['y'];n=len(x);mx=sum(x)/n;my=sum(y)/n;den=sum((v-mx)**2 for v in x)\n        if den==0:raise ValueError('ZERO_X_VARIANCE')\n        slope=sum((x[i]-mx)*(y[i]-my) for i in range(n))/den;intercept=my-slope*mx\n        sst=sum((v-my)**2 for v in y);sse=sum((y[i]-(slope*x[i]+intercept))**2 for i in range(n));r2=1.0-sse/sst if sst else 1.0\n        result={'slope':slope,'intercept':intercept,'r2':r2}\n    elif op=='monte_carlo_pi':\n        rng=random.Random(task['seed']);samples=int(task['samples']);inside=0\n        for _ in range(samples):\n            x=rng.random();y=rng.random();inside+=1 if x*x+y*y<=1.0 else 0\n        result={'samples':samples,'seed':int(task['seed']),'pi':4.0*inside/samples}\n    else:raise ValueError('UNSUPPORTED_TASK_OP')\n    encoded=json.dumps(result,separators=(',',':'),sort_keys=True)\n    return {'ok':True,'revision':TASK_REVISION,'task_id':str(task.get('task_id','')),'op':op,'result':result,'result_digest':hashlib.sha256(encoded.encode()).hexdigest()}\n\nt0=time.time();n=1000000\ncalc=sum(i*i for i in range(1,n+1));expected=n*(n+1)*(2*n+1)//6\ncpu=_cpu();mem=_mem()\nreceipt={'ok':bool(calc==expected and cpu>=${MIN_EFFECTIVE_CPU} and mem>=${MIN_EFFECTIVE_MEMORY_GIB}),'revision':REVISION,'cpu_effective':round(cpu,3),'memory_gib_effective':round(mem,3),'nominal_cpu':${TARGET_CPU},'nominal_memory_gb':${TARGET_MEMORY_GB},'square_sum_correct':calc==expected,'result_digest':hashlib.sha256(str(calc).encode()).hexdigest(),'python':platform.python_version(),'elapsed_s':round(time.time()-t0,6)}\nprint(MARKER+json.dumps(receipt,separators=(',',':'),sort_keys=True),flush=True)\n\ntask_receipt=None\nraw_task=os.environ.get(TASK_VAR,'').strip()\nif raw_task:\n    task={}\n    tt=time.time()\n    try:\n        task=json.loads(raw_task);task_receipt=_task(task);task_receipt['elapsed_s']=round(time.time()-tt,6)\n    except Exception as exc:\n        task_receipt={'ok':False,'revision':TASK_REVISION,'task_id':str(task.get('task_id','')),'op':str(task.get('op','')),'result':None,'result_digest':None,'elapsed_s':round(time.time()-tt,6),'error_class':type(exc).__name__}\n    print(TASK_MARKER+json.dumps(task_receipt,separators=(',',':'),sort_keys=True),flush=True)\n\ndef status():return json.dumps({'runtime':receipt,'task':task_receipt},separators=(',',':'),sort_keys=True)\n\nimport gradio as gr\ndemo=gr.Interface(fn=status,inputs=None,outputs='text',title='Three Center ModelScope Lite CPU')\ndemo.launch(server_name='0.0.0.0',server_port=7860)\n`}
...
export const modelScopeStudioLiteMeta=()=>({provider:"modelscope",repo_name:REPO_NAME,sdk_type:SDK_TYPE,target_hardware:TARGET_HARDWARE,nominal_cpu:TARGET_CPU,nominal_memory_gb:TARGET_MEMORY_GB,min_effective_cpu:MIN_EFFECTIVE_CPU,min_effective_memory_gib:MIN_EFFECTIVE_MEMORY_GIB,revision:REVISION,task_revision:TASK_REVISION,task_variable:TASK_VAR,task_marker:TASK_MARKER,phased_runner:true,idempotent_upload:true,stop_requires_hardware_catalog:false,free_only:true,paid_fallback:false});
