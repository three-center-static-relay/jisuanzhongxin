...
async function acceptance(req,env){
  ...
  if(req.method==="GET")return json({ok:true,task:safeTask(current)});
  ...
  try{
    ...
    await save(env,{status:"bridge_fetching",bridge_stage:"result_polling",failure_class:null,error:null,bridge_ticket_digest:digest,bridge_ticket_expires_at_ms:Date.now()+600000,fetch_retry_at:now()});
