...
const internalOnly=u=>u.hostname==="compute.internal";
const deny=()=>error("POLICY_DENIED","Benchmark computational routes are service-binding internal only",403,{execution_started:false});
...
export async function maybeHandleBenchmarks(req){const u=new URL(req.url);try{
  ...
  if(req.method==="POST"&&u.pathname==="/v1/benchmarks/plan"){if(!internalOnly(u))return deny();return json({ok:true,plan:benchmarkPlan(await parse(req))})}
  if(req.method==="POST"&&u.pathname==="/v1/benchmarks/evaluate"){if(!internalOnly(u))return deny();const out=evaluateBenchmarks(await parse(req));return json(out,out.state==="red"?422:200)}
  if(req.method==="POST"&&u.pathname==="/v1/benchmarks/validate-reference-pack"){if(!internalOnly(u))return deny();return json(validateReferencePack(await parse(req)))}
  return null;
