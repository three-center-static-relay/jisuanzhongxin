...
export async function probeHuaweiOfficialSdkAuth(env={}){
  ...
  if(!parsed.ok||!ak||!sk)return{ok:false,configured:false,provider:"huawei-functiongraph-official-sdk",official_sdk_used:true,official_sdk_version:SDK_VERSION,sdk_runtime_compatible:null,http_status:0,authenticated:false,authorized:false,auth_detail_class:"PREFLIGHT_FAILED",upstream_called:false,route_eligible:false,paid_fallback:false,secret_echo:false};

  try{
    const credentials=new core.BasicCredentials()
      .withAk(ak)
      .withSk(sk)
      .withProjectId(parsed.project_id);
    ...
    return{ok:status>=200&&status<300,configured:true,provider:"huawei-functiongraph-official-sdk",official_sdk_used:true,official_sdk_version:SDK_VERSION,sdk_runtime_compatible:true,http_status:status,authenticated:status!==401,authorized:status>=200&&status<300,auth_detail_class:status>=200&&status<300?"AUTHENTICATED":"OTHER_AUTH_ERROR",function_list_present:Array.isArray(result?.functions),upstream_called:true,route_eligible:false,paid_fallback:false,secret_echo:false};
  }catch(error){
    const status=Number(error?.httpStatusCode||0);
    const code=String(error?.errorCode||"").slice(0,80)||null;
    const detail=detailClass(error?.errorMsg||error?.message);
    const authRejected=status===401||code==="APIGW.0301"||code==="APIG.0301";
    return{ok:false,configured:true,provider:"huawei-functiongraph-official-sdk",official_sdk_used:true,official_sdk_version:SDK_VERSION,sdk_runtime_compatible:status>0,http_status:status,authenticated:status>0&&!authRejected,authorized:false,upstream_error_code:code,auth_detail_class:detail,error_name:String(error?.name||"Error").slice(0,80),upstream_called:status>0,route_eligible:false,paid_fallback:false,secret_echo:false};
  }
}
