...
function retryCount(t){const n=Number(t?.retry_count);if(Number.isFinite(n)&&n>=0)return Math.trunc(n);return t?.retry_at?1:0}
function safeTask(t){return t?{task_id:TASK_ID,status:t.status||null,executor:t.executor||null,retry_count:retryCount(t),baidu_job_id_present:Boolean(t.baidu_job_id),bridge_stage:t.bridge_stage||null,failure_class:t.failure_class||null,shell_canary_executed:t.failure_class==="BAIDU_SHELL_CANARY_EXECUTED",bridge_result_retrieved:t.bridge_result_retrieved===true,error:t.error||null,finished_at:t.finished_at||null,circleci_pipeline_id:t.circleci_pipeline_id||null}:null}
...
async function retry(env){
  ...
  const retries=retryCount(current);if(retries>=2)return json({ok:false,error:"RETRY_LIMIT_REACHED",task:safeTask(current)},409);
  ...
  try{const ticket=newBridgeTicket(),digest=await digestBridgeTicket(ticket);await save(env,{status:"bridge_retry_dispatching",executor:"baidu-circleci-cli",retry_count:retries+1,bridge_stage:"cloudflare_dispatching",failure_class:null,error:null,finished_at:null,bridge_ticket_digest:digest,bridge_ticket_expires_at_ms:Date.now()+900000,retry_at:now(),official_runsh_fix:true});const out=await triggerBaiduBridge(env,{op:"SUBMIT",task_id:TASK_ID,bridge_ticket:ticket});await save(env,{status:"bridge_submitted",circleci_pipeline_id:out.pipeline_id,circleci_pipeline_number:out.pipeline_number,bridge_started_at:now()});return json({ok:true,task_id:TASK_ID,status:"bridge_retry_submitted",circleci_pipeline_id:out.pipeline_id,retry_count:retries+1,official_runsh_fix:true},202)}catch(e){await save(env,{status:"failed",failure_class:"CIRCLECI_RETRY_DISPATCH_FAILED",error:String(e?.message||e).slice(0,300),finished_at:now(),bridge_ticket_digest:null});await release(env).catch(()=>{});return json({ok:false,error:String(e?.message||"RETRY_DISPATCH_FAILED").slice(0,200)},502)}
}
