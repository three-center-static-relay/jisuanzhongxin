...
const PATH="/__acceptance/baidu-v100-inline-j-20260815j-31e8c2";
...
function safeFailureClass(v){const s=String(v||"").trim().toUpperCase();return/^[A-Z0-9_]{3,80}$/.test(s)?s:"BAIDU_INLINE_CANARY_INVALID_RESULT"}
...
async function structuredFailureCallback(req,env){
  ...
  const result=b.result||{};if(String(result.task_id||"")!==TASK_ID||String(result.profile||"")!=="gpu")return json({ok:false,error:"RESULT_IDENTITY_MISMATCH"},409);
  ...
  await save(env,{status:"failed",baidu_job_id:job||null,bridge_stage:"result_retrieved",error:failure_class,failure_class,verification:{ok:false,reason:failure_class,upstream_failure:true,failure_stage},structured_runtime_failure:true,bridge_result_retrieved:true,bridge_ticket_digest:null,finished_at:now()});await release(env).catch(()=>{});
...
async function start(env){
  ...
  const current=(await load(env)).task;if(current)return json({ok:current.status!=="failed",already_started:true,task:safeTask(current)},202);
  const lock=await acquire(env,540);if(!lock.ok)return json({ok:false,error:"BUSY",active:lock.active||null},409);
  try{const ticket=newBridgeTicket(),digest=await digestBridgeTicket(ticket);const manifest={task_id:TASK_ID,profile:"gpu",input:{diagnostic:"fixed-inline-startup-canary"},timeout_seconds:180};await save(env,{status:"bridge_dispatching",profile:"gpu",executor:"baidu-circleci-inline-canary",gpu:true,manifest,created_at:now(),cancel_requested:false,bridge_ticket_digest:digest,bridge_ticket_expires_at_ms:Date.now()+900000,acceptance_run:true,inline_canary:true,bridge_stage:"cloudflare_dispatching"});const out=await triggerBaiduBridge(env,{op:"SUBMIT",task_id:TASK_ID,bridge_ticket:ticket});await save(env,{status:"bridge_submitted",circleci_pipeline_id:out.pipeline_id,circleci_pipeline_number:out.pipeline_number,bridge_started_at:now()});return json({ok:true,task_id:TASK_ID,status:"bridge_submitted",circleci_pipeline_id:out.pipeline_id,device:"v100",gpus:1,payment:"coupon",diagnostic:"fixed-inline-startup-canary"},202)}catch(e){await save(env,{status:"failed",failure_class:"CIRCLECI_DISPATCH_FAILED",error:String(e?.message||e).slice(0,300),finished_at:now(),bridge_ticket_digest:null});await release(env).catch(()=>{});return json({ok:false,error:String(e?.message||"CIRCLECI_DISPATCH_FAILED").slice(0,200)},502)}
}
...
async function acceptance(req,env){const u=new URL(req.url);if(![PATH,CHECK_PATH].includes(u.pathname))return null;if(Date.now()>EXPIRES_AT)return json({ok:false,error:"ACCEPTANCE_ROUTE_EXPIRED"},410);if(req.method==="GET")return json({ok:true,bridge:baiduCircleCIMeta(env),task:safeTask((await load(env)).task)});if(req.method!=="POST")return json({ok:false,error:"METHOD_NOT_ALLOWED"},405);if(u.pathname===CHECK_PATH)return check(env);return start(env)}
