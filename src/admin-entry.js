...
export default{
  async fetch(req,env,ctx){
    ...
    if(req.method==="GET"&&url.pathname==="/v1/admin/context"){const denied=internalOnly(url,"admin context is service-binding internal only");if(denied)return denied;return adminContext(env,ctx)}
    if(req.method==="GET"&&url.pathname==="/v1/admin/autonomy"){const denied=internalOnly(url,"autonomy status is service-binding internal only");if(denied)return denied;return json(await getAutonomySnapshot(env))}
    if(req.method==="GET"&&url.pathname==="/v1/admin/modelscope"){const denied=internalOnly(url,"ModelScope runtime status is service-binding internal only");if(denied)return denied;return json(await getModelScopeRuntimeSnapshot(env))}
    if(req.method==="GET"&&url.pathname==="/v1/admin/modelscope/studio-lite/status"){const denied=internalOnly(url,"ModelScope Studio Lite status is service-binding internal only");if(denied)return denied;return internalStudioLite(env,"status")}
    if(req.method==="POST"&&url.pathname==="/v1/admin/modelscope/studio-lite/prepare"){const denied=internalOnly(url,"ModelScope Studio Lite prepare is service-binding internal only");if(denied)return denied;return internalStudioLite(env,"prepare")}
    if(req.method==="POST"&&url.pathname==="/v1/admin/modelscope/studio-lite/deploy"){const denied=internalOnly(url,"ModelScope Studio Lite deploy is service-binding internal only");if(denied)return denied;return internalStudioLite(env,"deploy")}
