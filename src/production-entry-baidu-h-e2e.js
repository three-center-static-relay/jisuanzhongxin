...
const PATH="/__acceptance/baidu-v100-e2e-20260815h-7d3a41";
...
function safeTask(t){return t?{task_id:TASK_ID,status:t.status||null,baidu_job_id_present:Boolean(t.baidu_job_id),bridge_stage:t.bridge_stage||null,failure_class:t.failure_class||null,verification_ok:t.verification?.ok===true,structured_runtime_failure:t.structured_runtime_failure===true,result_digest_present:/^[a-f0-9]{64}$/i.test(String(t.result_digest||"")),bridge_result_retrieved:t.bridge_result_retrieved===true,error:t.error||null,finished_at:t.finished_at||null,circleci_pipeline_id:t.circleci_pipeline_id||null}:null}
...
async function ticketAuthorized(req,t){
  const ticket=String(req.headers.get("x-three-center-bridge-ticket")||"").trim();
  if(!ticket||!t?.bridge_ticket_digest||Number(t.bridge_ticket_expires_at_ms||0)<Date.now())return false;
  const actual=await digestBridgeTicket(ticket),expected=String(t.bridge_ticket_digest);
...
async function structuredFailureCallback(req,env){
  ...
  if(!b||String(b.task_id||"")!==TASK_ID||String(b.status||"").toLowerCase()!=="completed"||b.result?.ok!==false)return null;
  const current=(await load(env)).task;if(!current)return null;
  if(!await ticketAuthorized(req,current))return json({ok:false,error:"UNAUTHORIZED"},401);
  const result=b.result||{};
  if(String(result.task_id||"")!==TASK_ID||String(result.profile||"")!==String(current.profile||"gpu"))return json({ok:false,error:"RESULT_IDENTITY_MISMATCH"},409);
  ...
  await save(env,{status:"failed",baidu_job_id:job||null,bridge_stage:"result_retrieved",error:failure_class,failure_class,verification:{ok:false,reason:failure_class,upstream_failure:true,failure_stage},structured_runtime_failure:true,bridge_ticket_digest:null,finished_at:now()});
...
async function start(env){
  ...
  try{
    ...
    await save(env,{status:"bridge_dispatching",profile:"gpu",executor:"baidu-circleci-cli",gpu:true,manifest,created_at:now(),cancel_requested:false,bridge_ticket_digest:digest,bridge_ticket_expires_at_ms:Date.now()+900000,acceptance_run:true,structured_runtime_result:true,bridge_stage:"cloudflare_dispatching"});
    ...
    return json({ok:true,task_id:TASK_ID,status:"bridge_submitted",circleci_pipeline_id:out.pipeline_id,device:"v100",gpus:1,payment:"coupon",submit_confirmation:"baidu-query-required",structured_runtime_result:true},202);
  }catch(e){
