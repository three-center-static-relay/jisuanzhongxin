...
const TASK_ID="baidu-circleci-live-20260815e";
const PATH="/__acceptance/baidu-v100-e2e-20260815e-6f2c91";
...
async function start(env){
  ...
  if(current)return json({ok:current.status!=="failed",already_started:true,task:safeTask(current)},current.status==="completed"?200:202);
  ...
  try{
    ...
    return json({ok:true,task_id:TASK_ID,status:"bridge_submitted",circleci_pipeline_id:out.pipeline_id,device:"v100",gpus:1,payment:"coupon"},202);
  }catch(e){
