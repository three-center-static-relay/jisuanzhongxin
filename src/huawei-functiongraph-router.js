...
import {probeHuaweiRawTls} from "./huawei-raw-tls-canary.js";
...
const RAW_TLS_AUDIT_PATH="/v1/providers/huawei-functiongraph/raw-tls-audit-42d7e91ac3";
...
async function authCanary(env){
  ...
  const value=await probeHuaweiDirectFunctionGraphAuthDetail(env);
...
export async function maybeHandleHuaweiFunctionGraph(req,env){
  ...
  if(req.method==="GET"&&url.pathname===RAW_TLS_AUDIT_PATH){
    const result=await probeHuaweiRawTls(env);
    return huaweiJson({...result,audit_scope:"one-shot-raw-tls-manual-http1"},result.authenticated===true?200:503);
  }
