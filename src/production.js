...
export default {async fetch(req,env,ctx){const u=new URL(req.url);
  ...
  if(req.method==="GET"&&u.pathname==="/v1/providers/baidu/meta")return json({ok:true,...baiduAIStudioMeta(),acceptance_state:"verified-auth-manual-cli",native_http_candidate:false,secret_echo:false});
  if(req.method==="GET"&&u.pathname==="/v1/providers/baidu/health"){const p=await baiduProbe(env);return json({...p,acceptance_state:p.manual_ready?"verified-auth-manual-cli":"not-ready",native_http_candidate:false},p.manual_ready?200:503)}
  if(req.method==="GET"&&u.pathname==="/v1/providers/kaggle/meta")return json({ok:true,provider:"kaggle",...kaggleOfficialMeta(),business_e2e:true,acceptance_state:"verified-current-cpu-t4-e2e",secret_echo:false});
