...
const DIAG_PATH="/__diag/baidu-circleci-live-20260815-a7c41e";
const DIAG_TASK_ID="baidu-circleci-live-20260815a";
...
async function liveAcceptance(req,env){
  ...
  if(req.method==="GET"){
    ...
    return json({ok:true,acceptance:"baidu-circleci-v100",configured:meta.configured,e2e_verified:meta.e2e_verified,task:t?{task_id:DIAG_TASK_ID,status:t.status||null,circleci_pipeline_id:t.circleci_pipeline_id||null,baidu_job_id_present:Boolean(t.baidu_job_id),verification:t.verification||null,result_digest:t.result_digest||null,error:t.error||null,bridge_result_retrieved:t.bridge_result_retrieved===true,finished_at:t.finished_at||null}:null});
  }
  ...
  if(old)return json({ok:true,task_id:DIAG_TASK_ID,status:old.status||"unknown",already_started:true,circleci_pipeline_id:old.circleci_pipeline_id||null},old.status==="completed"?200:202);
  ...
  try{
    ...
    await save(env,DIAG_TASK_ID,{status:"bridge_dispatching",profile:"gpu",executor:"baidu-circleci-cli",gpu:true,manifest,created_at:now(),cancel_requested:false,bridge_ticket_digest:ticketDigest,bridge_ticket_expires_at_ms:ticketExpiresAt,acceptance_run:true});
    ...
    await save(env,DIAG_TASK_ID,{status:"failed",error:String(e?.message||e).slice(0,300),bridge_ticket_digest:null,finished_at:now()});
