...
const CANARY_REVISION="qwen2.5-coder-math-v2-20260817";
...
export async function modelScopeInferenceCanary(env={}){
  ...
  try{
    ...
    return{ok:r.ok&&correct,provider:"modelscope",configured:true,authenticated:r.status!==401&&r.status!==403,inference_ok:r.ok&&correct,http_status:r.status,model:MODEL,canary_revision:CANARY_REVISION,expected:"323",correct,content_digest:content?await sha256(content):null,free_only:true,paid_fallback:false,secrets_redacted:true,error_class:r.ok?(correct?null:"UNEXPECTED_RESULT"):`MODELSCOPE_INFERENCE_HTTP_${r.status}`};
  }catch(e){return{ok:false,provider:"modelscope",configured:true,authenticated:false,inference_ok:false,http_status:e?.name==="AbortError"?504:null,model:MODEL,canary_revision:CANARY_REVISION,expected:"323",correct:false,content_digest:null,free_only:true,paid_fallback:false,secrets_redacted:true,error_class:e?.name==="AbortError"?"MODELSCOPE_INFERENCE_TIMEOUT":str(e?.message)||"MODELSCOPE_INFERENCE_FAILED"}}
