...
function nodeHttpsEmptyGet(url,headers){
  return new Promise((resolve,reject)=>{
    const target=new URL(url);
    const request=https.request({protocol:"https:",hostname:target.hostname,port:443,path:`${target.pathname}${target.search}`,method:"GET",headers},response=>{
...
export async function probeHuaweiNodeHttpsEmpty(env={}){
  ...
  if(!parsed.ok||!ak||!sk)return{ok:false,configured:false,provider:"huawei-functiongraph-node-https",transport:"node:https",request_semantics:"official-listfunctions-empty-get",body_bytes:0,content_length_sent:false,http_status:0,authenticated:false,authorized:false,auth_detail_class:"PREFLIGHT_FAILED",upstream_called:false,route_eligible:false,paid_fallback:false,secret_echo:false};
  ...
  try{
    ...
    const response=await nodeHttpsEmptyGet(url,{...baseHeaders,"x-sdk-date":signed.x_sdk_date,authorization:signed.authorization});
