...
export async function dispatch(env,task){
  const who=await introspect(env),kernelSlug=`three-center-${task.gpu?"t4":"cpu"}-${slugify(task.task_id)}`,fullSlug=`${who.username}/${kernelSlug}`,title=`Three Center ${task.gpu?"T4":"CPU"} ${String(task.task_id).slice(0,32)}`;
  const body={slug:fullSlug,newTitle:title,text:scriptFor(task),language:"python",kernelType:"script",isPrivate:true,enableInternet:false,sessionTimeoutSeconds:clamp(task.timeout_seconds,60,900,300)};
