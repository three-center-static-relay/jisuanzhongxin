...
async function run(req,env){
  ...
  try{
    if(localId){
      await g(env,`/task/${encodeURIComponent(taskId)}`,"POST",{status:"accepted",kind:"local-model",executor:"local-model",model_id:modelId,local_model_id:localId,created_at:now()});
      const result=runLocalModel(localId,args),resultDigest=await digest(JSON.stringify(result));
      await g(env,`/task/${encodeURIComponent(taskId)}`,"POST",{status:"completed",kind:"local-model",executor:"local-model",model_id:modelId,local_model_id:localId,result_digest:resultDigest,finished_at:now()});
    ...
    await g(env,`/task/${encodeURIComponent(taskId)}`,"POST",{status:"accepted",kind:"model-recipe",executor:"kaggle-official",profile:"model",model_id:modelId,recipe:recipe.recipe,method:recipe.method,gpu:false,created_at:now()});
    ...
    await g(env,`/task/${encodeURIComponent(taskId)}`,"POST",{status:"failed",model_id:modelId,error:String(e?.message||e),finished_at:now()}).catch(()=>{});await g(env,"/release","POST",{task_id:taskId}).catch(()=>{});
