...
async function callback(req,env){
  ...
  if(status==="running"||status==="cancel_requested"){
    await save(env,id,{status:status==="cancel_requested"?"cancel_requested":"running",baidu_job_id:job||null,last_bridge_callback_at:now()});
...
async function status(req,env){const b=await req.clone().json().catch(()=>({})),id=String(b.task_id||"");if(!id)return null;const t=(await load(env,id)).task;if(!t||t.executor!=="baidu-circleci-cli")return null;if(terminal(t.status))return json({ok:t.status!=="failed",task_id:id,status:t.status,result_digest:t.result_digest||null,verification:t.verification||null,finished_at:t.finished_at||null});return json({ok:true,task_id:id,status:t.status,baidu_job_id_present:Boolean(t.baidu_job_id),circleci_pipeline_id:t.circleci_pipeline_id||null,cancel_requested:t.cancel_requested===true,lock_released:false},202)}
