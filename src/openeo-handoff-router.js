...
function cleanError(e){return{ok:false,error:String(e?.message||"OPENEO_HANDOFF_FAILED"),message:"Copernicus Intelligence-to-openEO handoff failed",details:e?.details&&typeof e.details==="object"?e.details:undefined,execution_started:false,credits_spent:false,secret_echo:false}}
...
export async function maybeHandleOpenEOHandoff(req,env){
  ...
  try{
    ...
    if(u.pathname.endsWith("/plan"))return json(planOpenEOHandoff(input));
    const out=await validateOpenEOHandoff(env,input);
    return json(out,out.validation_ok===true?200:422);
