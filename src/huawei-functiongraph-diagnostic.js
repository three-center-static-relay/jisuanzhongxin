...
export async function probeHuaweiCredentialCrosscheck(env={}){
  ...
  try{
    ...
    if(!iam.iam_authenticated){
      return{ok:true,configured:true,provider:"huawei-credential-crosscheck",service:"iam",canary:"cts-readonly-auth",phase:"iam-project-discovery",http_status:iam.response.status,authenticated:false,iam_authenticated:false,project_context_match:false,region_project_found:false,region_project_count:0,authorized:false,upstream_error_code:iam.code,error_class:classifyHuaweiError(iam.code,upstreamMessage(iam.body)),region:parsed.region,route_eligible:false,paid_fallback:false,secret_echo:false};
    }
